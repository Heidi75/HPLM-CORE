...
export default function HPLM_Interface() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({
    // FORCE PATH CHECK: Ensure this matches your folder: app/api/chat/route.ts
    api: '/api/chat', 
    onResponse: (response) => {
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#0f0', minHeight: '100vh', padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', borderBottom: '1px solid #222', paddingBottom: '10px' }}>
        ...
        <button onClick={downloadFullAudit} disabled={messages.length === 0} style={{ backgroundColor: '#d4af37', color: '#000', border: 'none', padding: '5px 10px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
        ...
        <div style={{ border: '1px solid #0070f3', padding: '15px', marginBottom: '80px', backgroundColor: '#000510' }}>
          ...
          <div style={{ fontSize: '13px' }}>
            {messages.length === 0 ? (
              ...
              messages.map((m, i) => (
                <pre key={i} style={{ whiteSpace: 'pre-wrap', color: m.role === 'user' ? '#fff' : '#0f0', borderLeft: '2px solid #333', paddingLeft: '10px', marginBottom: '10px' }}>
                  {m.content}
                </pre>
              ))
            )}
